...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      const sevenDaysAgo = new Date(); // <-- TAMBAHKAN BARIS INI
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
        // Gunakan Promise.all untuk menjalankan semua query secara paralel
        const [userCount, messageCount, faqCount, newUsersCount] = await Promise.all([
          // Query untuk menghitung total pengguna di tabel 'profiles'
          supabase.from("profiles").select("*", { count: "exact", head: true }),
          // Query untuk menghitung total pesan di tabel 'kontak'
          supabase.from("kontak").select("*", { count: "exact", head: true }),
          // Query untuk menghitung total FAQ di tabel 'faq'
          supabase.from("faq").select("*", { count: "exact", head: true }),
          supabase
          .from('profiles')
          .select('*', { count: 'exact', head: true })
          .gte('created_at', sevenDaysAgo.toISOString()) // Filter berdasarkan tanggal
          
        ]);

        // Cek jika ada error pada masing-masing query
        if (userCount.error) throw userCount.error;
        if (messageCount.error) throw messageCount.error;
        if (faqCount.error) throw faqCount.error;
        if (newUsersCount.error) throw newUsersCount.error; // Jangan lupa cek errornya

        // Update state dengan data yang berhasil diambil
        setStats({
          totalUsers: userCount.count,
          totalMessages: messageCount.count,
          totalFaqs: faqCount.count,
          newUsersLast7Days: newUsersCount.count,
        });
      } catch (err) {
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-800/60 to-[#483DFF]/70 p-6 flex items-center">
          <div>
            ...
            <p className="text-gray-200 max-w-xl">
              Kelola sistem, pantau statistik, dan atur seluruh aktivitas
              platform keuangan secara efisien.
            </p>
          </div>
        </div>
      </div>
...
<StatCard title="Akun Terdaftar" value={loading ? "..." : stats.totalUsers} color="border-[#D4FF00]" icon={<FiUsers />}>
  {/* Tampilkan data dinamis dan hanya jika loading sudah selesai */}
  {!loading && (
    <p className="text-sm text-gray-500 mt-2">
      +{stats.newUsersLast7Days} dalam 7 hari terakhir
    </p>
  )}
</StatCard>
